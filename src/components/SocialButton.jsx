...
function SocialButton({ setting = {} }) {
  const [open, setOpen] = useState(false);

  // Collect active contact links
  const links = [];
  if (setting?.whatsapp) {
    links.push({
      type: "whatsapp",
      href: `https://wa.me/88${setting.whatsapp}`,
      label: "WhatsApp",
      external: true,
      icon: (
        <svg
  ...
  // If only one link, show directly without toggle
  if (links.length === 1) {
    const link = links[0];
    return (
      <div className="fixed-contact-wrapper">
        <div className="fixed-contact-buttons show single-mode">
          <a
            href={link.href}
            target={link.external ? "_blank" : undefined}
            rel={link.external ? "noreferrer" : undefined}
            className={`social-outside ${link.type} single-mode`}
          >
            <div className="text-card">{link.label}</div>
            <div className="wp-icon-circle">{link.icon}</div>
          </a>
        </div>
      </div>
    );
  ...
  return (
    <div className="fixed-contact-wrapper">
      <div
        className={`fixed-contact-buttons${open ? " show" : ""}`}
        id="contactActions"
      >
        {links.map((link) => (
          <a
            ...
            target={link.external ? "_blank" : undefined}
            rel={link.external ? "noreferrer" : undefined}
            className={`social-outside ${link.type}`}
          >
            <div className="text-card">{link.label}</div>
            <div className="wp-icon-circle">{link.icon}</div>
          </a>
      ...
      <a
        href="#"
        id="mainToggle"
        className="toggle-button call-toggle"
        onClick={(e) => {
          e.preventDefault();
          setOpen((o) => !o);
        }}
